...
import vigna.fastutil.BidirectionalIterator;

import java.util.*;

/** A type-specific red-black tree set with a fast, small-footprint implementation.
 *
 * <P>The iterators provided by this class are type-specific {@link BidirectionalIterator bidirectional iterators}. Moreover, the iterator returned by <code>iterator()</code> can
 * be safely cast to a type-specific {@linkplain ListIterator list iterator}. */
public class IntRBTreeSet extends AbstractIntSortedSet implements java.io.Serializable, Cloneable, IntSortedSet {
	...
	private class SetIterator extends AbstractIntListIterator {
		/** The entry that will be returned by the next call to {@link ListIterator#previous()} (or <code>null</code> if no previous entry exists). */
		Entry prev;
		/** The entry that will be returned by the next call to {@link ListIterator#next()} (or <code>null</code> if no next entry exists). */
		Entry next;
		/** The last entry that was returned (or <code>null</code> if we did not iterate or used {@link #remove()}). */
		Entry curr;
		/** The current index (in the sense of a {@link ListIterator}). Note that this value is not meaningful when this iterator has been created using the nonempty constructor. */
		int index = 0;
	 ...
	 * that the limits of the range are not any longer in the main set. Thus, things such as {@link SortedSet#first()} or {@link Collection#size()} must be always computed
	 * on-the-fly. */
	private final class Subset extends AbstractIntSortedSet implements java.io.Serializable, IntSortedSet {
		 ...
		 *
		 * <P>This class inherits from {@link SetIterator}, but overrides the methods that update the pointer after a {@link ListIterator#next()} or {@link ListIterator#previous()}
