...
import vigna.fastutil.HashCommon;
import vigna.fastutil.Size64;

import java.util.Collection;
import java.util.Iterator;
import java.util.NoSuchElementException;

import static vigna.fastutil.HashCommon.bigArraySize;
import static vigna.fastutil.HashCommon.maxFill;
 ...
 * {@linkplain BigArrays big array}) is filled up to a specified <em>load factor</em>, and then doubled in size to accommodate new entries. If the table is emptied below <em>one
	...
	protected final void shiftKeys( long pos ) {
		...
		for ( ;; ) {
			...
			for ( ;; ) {
				...
				slot = ( HashCommon.mix( (long)( ( IntBigArrays.get( key, pos ) ) ) ) ) & mask;
		...
		private final void shiftKeys( long pos ) {
			...
			for ( ;; ) {
				...
				for ( ;; ) {
					...
					slot = ( HashCommon.mix( (long)( ( curr ) ) ) ) & mask;
					if ( last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos ) break;
					pos = ( pos + 1 ) & mask;
				}
