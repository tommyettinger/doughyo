...
import vigna.fastutil.BigArrays;
...
public class ByteBigArrays {
	...
	public static byte[][] newBigArray( final long length ) {
		...
		final int baseLength = (int)( ( length + BigArrays.SEGMENT_MASK ) / BigArrays.SEGMENT_SIZE );
		byte[][] base = new byte[ baseLength ][];
		final int residual = (int)( length & BigArrays.SEGMENT_MASK );
		if ( residual != 0 ) {
	...
	public static byte[][] wrap( final byte[] array ) {
		...
		if ( array.length <= BigArrays.SEGMENT_SIZE ) return new byte[][] { array };
	...
	public static byte[][] ensureCapacity( final byte[][] array, final long length, final long preserve ) {
		...
		if ( length > oldLength ) {
			final int valid = array.length - ( array.length == 0 || array.length > 0 && array[ array.length - 1 ].length == BigArrays.SEGMENT_SIZE ? 0 : 1 );
			final int baseLength = (int)( ( length + BigArrays.SEGMENT_MASK ) / BigArrays.SEGMENT_SIZE );
			...
			if ( residual != 0 ) {
				...
				base[ i ] = new byte[ BigArrays.SEGMENT_SIZE ];
			if ( preserve - ( valid * (long) BigArrays.SEGMENT_SIZE ) > 0 ) copy( array, valid * (long) BigArrays.SEGMENT_SIZE, base, valid * (long) BigArrays.SEGMENT_SIZE, preserve - ( valid * (long) BigArrays.SEGMENT_SIZE ) );
			return base;
		}
		return array;
	}
