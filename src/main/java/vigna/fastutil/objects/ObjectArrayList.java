 ...
 */
package vigna.fastutil.objects;
 ...
 * {@link ObjectArrays} apply here. Moreover, you must take into consideration that assignment to an array not of type {@code Object[]} is slower due to type checking.
 *
...
public class ObjectArrayList<K> extends AbstractObjectList<K> implements RandomAccess, Cloneable, java.io.Serializable {
	...
	private void grow( final int capacity ) {
		if ( wrapped ) a = ObjectArrays.grow( a, capacity, size );
			...
			if ( capacity > a.length ) {
				final int newLength = (int) Math.max( Math.min( 2L * a.length, vigna.fastutil.Arrays.MAX_ARRAY_SIZE ), capacity );
	...
	public void removeElements( final int from, final int to ) {
		vigna.fastutil.Arrays.ensureFromTo( size, from, to );
